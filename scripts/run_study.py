...
- statistical significance (paired DM-style test on per-surface RMSE)
...
def run_study(
    ...
    for s_idx in range(n_surfaces):
        ...
        surfaces_params.append({"H": H, "eta": eta, "rho": rho, "xi0": xi0, "seed": s_idx})
    ...
    for s_idx, params in enumerate(surfaces_params):
        ...
        for seed in range(seeds_per_surface):
            # ArbNet -- use lr=1e-2 (we need to fit absolute Nifty-scale prices)
            arbnet_cfg = default_arbnet_config()
            arbnet = ArbNet(arbnet_cfg)
            ...
            rec_a.update({"surface": s_idx, "seed": seed, **params})
            ...
            rec_b.update({"surface": s_idx, "seed": seed, **params})
            ...
            rec_c.update({"surface": s_idx, "seed": seed, **params})
            all_records.append(rec_c)

            print(f"  seed {seed}: ArbNet RMSE={rec_a['price_rmse']:.2f} viol={rec_a['butterfly_rate']:.3%}/{rec_a['calendar_rate']:.3%}  "
                  f"Ackerer RMSE={rec_b['price_rmse']:.2f} viol={rec_b['butterfly_rate']:.3%}/{rec_b['calendar_rate']:.3%}  "
                  f"BS RMSE={rec_c['price_rmse']:.2f}")
    ...
    # Aggregate: per-model means and stds
    by_model = {"ArbNet": [], "Ackerer": [], "BS": []}
    ...
    # Compute paired differences in price RMSE per (surface, seed) and run a DM-style t-test.
