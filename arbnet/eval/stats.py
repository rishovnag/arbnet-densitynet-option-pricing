...
def moving_block_bootstrap_ci(
    ...
    """Moving-block bootstrap CI for serially correlated series (M2).

    The i.i.d. percentile bootstrap in :func:`bootstrap_ci` understates the
    variance of a mean over autocorrelated daily losses (adjacent trading
    days share overlapping cross-sections), giving optimistically narrow CIs
    -- inconsistent with the HAC correction the DM test applies. This variant
    resamples overlapping blocks of length ``block_len`` (default: the
    Newey-West rule-of-thumb bandwidth ``floor(4 (n/100)^{2/9}) + 1``, i.e.
    the same dependence scale the DM test assumes).
    ...
    n = len(data)
    if n < 3:
        return float(statistic(data)), float("nan"), float("nan")
    if block_len is None:
        block_len = int(np.floor(4.0 * (n / 100.0) ** (2.0 / 9.0))) + 1
    ...
    for b in range(n_boot):
        ...
        sample = np.concatenate([data[s:s + block_len] for s in starts])[:n]
