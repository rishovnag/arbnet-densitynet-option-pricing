    ...
    for epoch in range(cfg.n_epochs):
        ...
        for i in range(0, n, cfg.batch_size):
            ...
            if cfg.lambda_arb > 0 and soft_penalty_grid is not None:
                # Pass a single context row so the penalty grid is evaluated in the
                # same context as the fit (required for a context-enabled baseline;
                # no-op when the model is context-free).
                ctx_pen = None
                if "context" in batch and batch["context"] is not None:
                    ctx_pen = batch["context"][0]
