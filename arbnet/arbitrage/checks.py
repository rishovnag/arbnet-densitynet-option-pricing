...
2. Calendar: evaluated as e^{(r-q)T} C(K, T) non-decreasing in T at FIXED
   STRIKE. Scope note (H5): the architecture guarantees e^{rT} C non-decreasing
   (Theorem 4.1, r >= q); because e^{(r-q)T} C = e^{-qT} (e^{rT} C) with
   e^{-qT} decreasing, passing this check IMPLIES passing the guaranteed
   condition (it is strictly harder), but it is not itself guaranteed for
   q > 0. The exact model-free condition -- the normalized undiscounted call
   non-decreasing in T at fixed forward-moneyness kappa = K/F_T (equivalently
   total variance non-decreasing at fixed forward log-moneyness) -- is
   implemented in ``arbnet.arbitrage.adversarial`` and reported separately.
...
def calendar_violations_price(
    ...
    Checks e^{(r-q)T} * C(K, T) non-decreasing in T at fixed strike K, which
    reduces to e^{rT} * C when q = 0. For q > 0 this is STRICTER than the
    architecturally guaranteed condition (e^{rT} C non-decreasing): passing it
    implies the guarantee holds, but a violation here does not by itself breach
    Theorem 4.1. The exact model-free calendar condition is at fixed
    forward-moneyness -- see ``adversarial_arbitrage_report``.
...
def static_arbitrage_report(
    ...
    non-decreasing in T at fixed K). For q > 0 this is a STRICTER surrogate
    than the architecturally guaranteed e^{rT} C monotonicity -- see the
    module docstring (H5) -- so a zero here certifies the guaranteed condition
    as well; report the exact fixed-forward-moneyness condition via
    ``adversarial_arbitrage_report`` alongside. The tail check is performed in
    implied-total-variance space via Roger Lee's bound.
