...
from .composite import bs_call_price
...
class HestonPricer(nn.Module):
    ...
    def _char_func(self, u: torch.Tensor, T: torch.Tensor) -> torch.Tensor:
        ...
        # "Little Heston trap" branch (Albrecher et al. 2007): the STABLE form
        # uses g = (xi - d)/(xi + d) with e^{-dT}. The previous code used the
        # inverted g = (xi + d)/(xi - d), which grows without bound (|phi| up
        # to ~34, NaN at u->0) and made every Heston price garbage that the
        # output clamp then silently masked to zero.
        g = (xi - d) / (xi + d)
        exp_dT = torch.exp(-d * T_c)
        C = kappa * theta / (sigma**2) * ((xi - d) * T_c - 2.0 * torch.log((1.0 - g * exp_dT) / (1.0 - g)))
    ...
    def forward(self, K, T, S, r, q=None, context=None):
        ...
        # A materially negative price signals a CF-branch/quadrature bug, not
        # round-off; raise instead of silently clamping it to zero (the old
        # clamp masked the inverted-trap-branch bug for every Heston price).
        if bool((price < -1e-6 * S_d).any()):
            raise FloatingPointError(
                f"Heston price materially negative (min {float(price.min()):.4g}); "
                "characteristic-function branch or quadrature is broken")
        price = price.clamp(min=0.0).to(S.dtype)   # tiny quadrature round-off only
